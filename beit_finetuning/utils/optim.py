...
def get_optim_for_finetune(__C, model, new_params_name='head'):
    # Optimizer for new parameters (output layer)
    optim_class1 = eval('torch.optim.' + __C.OPT_FTW)
    params1 = []
    for name, param in model.named_parameters():
    ...
    # Optimizer for pre-trained and other parameters
    optim_class2 = eval('torch.optim.' + __C.OPT)
    params2 = [
        {'params': [], 'lr': __C.LR_BASE, '_grad_norm_clip': __C.GRAD_NORM_CLIP},
        {'params': [], 'lr': __C.LR_BASE * __C.BERT_LR_MULT, '_grad_norm_clip': __C.GRAD_NORM_CLIP},
    ]
    for name, param in model.named_parameters():
        if param.requires_grad:
            ...
            elif new_params_name in name:
                continue  # Skip parameters already in optimizer1
            else:
